...
# Assuming input_array and output_array are your data arrays
input_array = np.array([30, 30, 30, 30, 30])  # Replace with your actual input data
output_array = np.array([0, 1, 2, 3, 4])    # Replace with your actual output data
...
X = np.array(X).reshape(-1, 1, 2)  # Reshape for RNN input (samples, time steps, features)
y = np.array(y)

# Define the stacked RNN model
model = tf.keras.Sequential([
    tf.keras.layers.LSTM(64, return_sequences=True, input_shape=(1, 2)),  # First LSTM layer
